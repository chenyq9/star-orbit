...
> 更新时间：2026-09-24（v1.5.1 22球满盘修正完成；40关版 40/60 就绪）
...
8. 【已完成 2026-09-24】22球阶梯修正（v1.5.1，commit 3201eb2）：L31-40 改 6→22 球阶梯重产重植入（L40=circle.html 完全体 b10+o10+p2），k传参bug同轮修复（显式K+防抵消+path落盘），验收103项+回归30/30+自检10/10 全通过。遗留：①Shizuku恢复后浏览器层补验（16/20/22球画面拥挤度）；②满盘关提示按钮 solveHint BFS 对22球会溢出（CAP 30万态触发前冻结数秒UI）——H1/H2/H3 需改用回退path逆序做提示源，已列设计待办
